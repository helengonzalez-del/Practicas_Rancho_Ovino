...
async function buildChartPesos() {
  ...
  const label = animalSeleccionado
    ? (() => {
        ...
        return sel?.options[sel.selectedIndex]?.text || 'Animal';
      })()
    : 'Todos';
  ...
  buildChart('chartPesos', 'line', meses,
    ...
    }],
    {
      scales: {
        y: {
          beginAtZero: false,
          ticks: { color: '#4A7A5A', callback: v => v + ' kg' }
        },
        x: { ticks: { color: '#4A7A5A' } }
      },
      plugins: {
        legend: { labels: { font: { family: 'Lato' }, color: '#0F3320' } },
        tooltip: {
          callbacks: {
            label: ctx => ctx.parsed.y !== null ? ctx.parsed.y + ' kg' : 'Sin dato'
          }
        }
      }
    }
